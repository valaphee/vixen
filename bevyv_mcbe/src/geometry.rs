...
impl AssetLoader for GeometryLoader {
    fn load<'a>(&'a self, bytes: &'a [u8], load_context: &'a mut LoadContext) -> BoxedFuture<'a, anyhow::Result<()>> {
        Box::pin(async move {
            for geometry in serde_json::from_slice::<GeometryFile>(bytes)?.geometry {
                ...
                for bone in &geometry.bones {
                    ...
                    if let Some(cubes) = &bone.cubes {
                        for cube in cubes {
                            ...
                            let quat_rot = Quat::from_euler(EulerRot::XYZ, -euler_rot.x.to_radians(), -euler_rot.y.to_radians(), -euler_rot.z.to_radians());
                            transform.rotate_around(cube.pivot.unwrap_or(Vec3::ZERO), quat_rot);

                            let inflate = cube.inflate.or(bone.inflate).unwrap_or(0.);
                            let Vec3 { x: size_x, y: size_y, z: size_z } = cube.size + Vec3::splat(inflate);

                            fn get_face_uv(cube: &GeometryBoneCube, face: GeometryBoneCubeFace) -> Option<(Vec2, Vec2)> {
                                ...
                                match &cube.uv {
                                    GeometryBoneCubeUv::Box(uv) => Some(match face {
                                        GeometryBoneCubeFace::North => (Vec2::new(cube.size.z, cube.size.z) + uv.clone(), uv_size),
                                        GeometryBoneCubeFace::West => (Vec2::new(0., cube.size.z) + uv.clone(), uv_size),
                                        GeometryBoneCubeFace::South => (Vec2::new(cube.size.z + cube.size.x + cube.size.z, cube.size.z) + uv.clone(), uv_size),
                                        GeometryBoneCubeFace::East => (Vec2::new(cube.size.z + cube.size.x, cube.size.z) + uv.clone(), uv_size),
                                        GeometryBoneCubeFace::Up => (Vec2::new(cube.size.z, 0.) + uv.clone(), uv_size),
                                        GeometryBoneCubeFace::Down => (Vec2::new(cube.size.z + cube.size.x, 0.) + uv.clone(), uv_size)
                                    }),
                                    GeometryBoneCubeUv::PerFace(uv_by_face) => uv_by_face.get(&face).map(|uv| { (uv.uv, uv.uv_size.unwrap_or(uv_size)) })
                                }
                            }

                            if let Some(face_uv) = get_face_uv(cube, GeometryBoneCubeFace::North) {
                                positions.push(transform.transform_point(Vec3::from([-inflate, -inflate, size_z])).to_array());
                                positions.push(transform.transform_point(Vec3::from([size_x, -inflate, size_z])).to_array());
                                positions.push(transform.transform_point(Vec3::from([size_x, size_y, size_z])).to_array());
                                positions.push(transform.transform_point(Vec3::from([-inflate, size_y, size_z])).to_array());
                            ...
                            if let Some(face_uv) = get_face_uv(cube, GeometryBoneCubeFace::South) {
                                positions.push(transform.transform_point(Vec3::from([-inflate, -inflate, -inflate])).to_array());
                                positions.push(transform.transform_point(Vec3::from([-inflate, size_y, -inflate])).to_array());
                                positions.push(transform.transform_point(Vec3::from([size_x, size_y, -inflate])).to_array());
                                positions.push(transform.transform_point(Vec3::from([size_x, -inflate, -inflate])).to_array());
                    ...
                    if let Some(poly_mesh) = &bone.poly_mesh {
                        match &poly_mesh.polys {
                            GeometryBonePolyMeshPolys::Tri(primitives) => for primitive in primitives {
                            ...
                            GeometryBonePolyMeshPolys::Quad(primitives) => for primitive in primitives {
                                for vertex in primitive {
                                    positions.push(poly_mesh.positions[vertex.x as usize].to_array());
                    ...
                    bones.insert(bone.name.clone(), load_context.set_labeled_asset(&bone.name, LoadedAsset::new(mesh)));
                }
                load_context.set_labeled_asset(&geometry.description.identifier, LoadedAsset::new(Skeleton(bones)));
            }
...
#[derive(Serialize, Deserialize)]
pub struct GeometryBoneCubeFaceUv {
...
#[derive(Serialize, Deserialize)]
pub struct GeometryBonePolyMesh {
...
#[derive(Serialize, Deserialize)]
#[serde(untagged)]
pub enum GeometryBonePolyMeshPolys {
...
#[derive(Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum GeometryBonePolyMeshPolysType {
    TriList,
    QuadList,
}

#[derive(Serialize, Deserialize)]
pub struct GeometryBoneTextureMesh {
